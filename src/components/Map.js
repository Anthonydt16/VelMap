            ...
            if (contratsJson.length > 0) {
                ...
                for (const contratUnit of contratFilter) {
                    ...
                    if (!contratUnit.latitude || !contratUnit.longitude) {
                        ...
                        markers.push(<Marker position={[contratUnit.latitude, contratUnit.longitude]}><Popup><p>le nom: {contratUnit.name}</p><p>le pays: {contratUnit.country_code}</p><p>le nom commercial: {contratUnit.commercial_name}</p></Popup></Marker>);
                    }
